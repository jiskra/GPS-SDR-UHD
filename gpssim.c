...
void computeCodePhase(channel_t *chan, range_t rho1, double dt)
{
	...
	// Pseudorange rate.
	rhorate = (rho1.range - chan->rho0.range)/dt; //use the last range and the current rang to calculate the Carrier and code frequency
		...
		if (interactive)
		{
			...
			if(_kbhit())
			{
				...
				case NORTH_KEY:
					...
					if (velocity[0]<MAX_VEL||velocity[0]>-MAX_VEL){
						velocity[0] += DEL_VEL;
						printf("\r                                   Accelerate Velocity is NS%c:%3.3f    WE%c:%3.3f  UD%c:%3.3f",
								(velocity[0]>0)?24:25,velocity[0],(velocity[1])>0?26:27,velocity[1],(velocity[2]>0)?30:31,velocity[2]);
						}
				...
				case UP_KEY:
					key_direction = UP;
					if (velocity[2]<MAX_VEL||velocity[2]>-MAX_VEL){
						velocity[2] += DEL_VEL;
						printf("\r                                   Accelerate Velocity is NS%c:%3.3f    WE%c:%3.3f  UD%c:%3.3f",
								(velocity[0]>0)?24:25,velocity[0],(velocity[1])>0?26:27,velocity[1],(velocity[2]>0)?30:31,velocity[2]);
						}
				...
				case DOWN_KEY:
					...
					if (velocity[2]<MAX_VEL||velocity[2]>-MAX_VEL){
						velocity[2] -= DEL_VEL;
						printf("\r                                    Accelerate Velocity is NS%c:%3.3fWE%c:%3.3fUD%c:%3.3f",
								(velocity[0]>0)?24:25,velocity[0],(velocity[1])>0?26:27,velocity[1],(velocity[2]>0)?30:31,velocity[2]);
						}
					break;
				default:
			...
			if ((velocity[0]!=0.0)||(velocity[1]!=0.0)||(velocity[2]!=0.0))
			{
				// Update the user location
					neu[0] = velocity[0] * 0.1;
					neu[1] = velocity[1] * 0.1;
					neu[2] = velocity[2] * 0.1;
